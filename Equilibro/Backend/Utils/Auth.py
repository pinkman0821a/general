...
def hashPassword(contrasena: str) -> str:
    """hash a password for storing."""
    salt = bcrypt.gensalt()
    hashed = bcrypt.hashpw(contrasena.encode('utf-8'), salt)
    return hashed.decode('utf-8')

def verifyPassword(contrasena: str, hashed: str) -> bool:
    """check hashed password. Using bcrypt, the salt is saved into the hash itself"""
    return bcrypt.checkpw(contrasena.encode('utf-8'), hashed.encode('utf-8'))

def validateRegistration(nombreusuario: str, email: str, contrasena: str) -> tuple[bool, str]:
...
def generateJwtToken(nombreusuario: str, correo: str, usuario_id: int, secret_key: str, expires_in: int ) -> str:
